...
| 2026-08-12 | **Confidence made per-object, not per-method.** A live run showed every single prediction landing at Tier 1 — real given the code, but misleading: `METHOD_CONFIDENCE` was a flat lookup keyed only by *which* method fired (category/function/keyword/default), so every wall classified the same way got an identical score regardless of anything specific to that wall. Feedback: confidence should be per object. Reworked `_heuristic_predict()` into `_heuristic_signals()` (collects every independent signal that fires — category, Function param, type-name/family keyword — instead of returning on the first match) + `_heuristic_predict()` (picks the strongest signal for the code/method as before, then adjusts its base confidence per-wall: `CORROBORATION_BONUS` if a weaker signal agrees on the same code, `CONFLICT_PENALTY` if one contradicts it, capped below 1.0 via `CORROBORATION_CAP` — still a heuristic, never a genuine reference-wall match). Caught and fixed a self-corroboration bug while building this: the keyword-search text originally included `wall.function`, and every `FUNCTION_TO_CODE` keyword (exterior/interior/retaining/foundation/curtain) also exists verbatim in `HEURISTIC_MAP` mapped to the same code — so a Function match would almost always "corroborate" itself via the same underlying word restated, not a genuinely independent second signal. Fixed by scoping the keyword search to `type_name + family` only, so corroboration reflects what the wall was actually *named*, checked against what Revit's Function parameter says. A wall where category/Function/name all agree now scores higher than one with a single weak signal; a wall where they contradict (e.g. category says curtain wall but Function says "Interior") now visibly drops a tier — flagging a real data inconsistency instead of hiding it behind a flat per-method score. Added `tests/test_predict_codes.py::TestPerObjectConfidenceAdjustment` (single-signal baseline, agreement, conflict, and the corroboration cap) and updated the three existing tests whose fixtures happened to have an extra keyword match. |
| 2026-08-12 | **Run status message rewritten to lead with outcome, not a raw tally.** Feedback: the summary read as a dry inventory ("Processed N elements — X already Level 4, Y remapped, Z predicted — N total predictions (...)") and undersold what actually happened. Rewrote it to lead with the transformation ("Auto-conditioned all N wall elements to Turner Uniformat Level 4 in one pass"), fold the existing/remapped/predicted breakdown into that sentence, and replace the flat Tier 1 count with a plain-language confidence read-out ("every prediction landed at Tier 1 — no manual triage needed", or names how many didn't if not). Still fully backed by the same real counts — just framed as what got accomplished instead of what got counted. |
| 2026-08-12 | **Raised TIER_1_THRESHOLD from 0.75 to 0.85.** Feedback: 75% feels low for a tier documented as "high confidence, candidate for auto-accept" — and it was: 0.75 was set to exactly equal `heuristic_function`'s base confidence, so a single, uncorroborated Revit Function-parameter match alone always cleared Tier 1, with nothing else on the wall confirming it. Moved the threshold to 0.85 — `heuristic_category`'s base — so Tier 1 now means either Revit's own authoritative category assignment on its own, or a heuristic match independently corroborated by a second signal on the same wall (0.75 + `CORROBORATION_BONUS` 0.10 = 0.85). A bare `heuristic_function` match now lands Tier 2 instead. Updated the two `test_imprint_predictions.py` cases that used a lone Function match (now correctly assert Tier 2) and `TestConfidenceToTier`'s boundary tests; added explicit tier assertions to `TestPerObjectConfidenceAdjustment`'s single-signal and agreeing-signals cases so the "lone signal → Tier 2, corroborated → Tier 1" behavior this was built for is actually covered, not just implied. |
| 2026-08-12 | **Tier 3 surfaced explicitly, not folded into "Tier 2/3".** Feedback: genuinely Tier 3 predictions (low/no confidence — the tier that actually means "a human needs to look at this one") are worth knowing about on their own, not lumped in with Tier 2 ("quick check"). Three places updated: (1) `main.py`'s status headline now breaks out Tier 1/2/3 counts separately and only says "genuinely need a closer look" when Tier 3 > 0, instead of a blanket "Tier 2/3" phrase; (2) `report.py` gained a "Needs a Closer Look (Tier 3)" section right after the Predictions table — lists the specific elements if any exist, says "None" if not, rather than requiring someone to scroll a big confidence-sorted table to check; (3) `speckle_io.attach_viewer_annotations()` now fires a dedicated `attach_warning_to_objects` (not `attach_info`) call for every Tier 3 prediction, grouped by predicted code across all methods — a real severity distinction in the run report's Results list, not just a "Tier 3" substring buried inside a longer info label. Added `tests/test_attach_viewer_annotations.py` (hand-rolled fake context, no live Speckle call) covering both the warning-fires and warning-doesn't-fire cases. |
| 2026-08-12 | **Investigated "0 already Turner Level 4" — verified correct, but found and fixed a related latent gap.** Feedback: suspected there might be existing Level 4 codes not in dot-notation format being missed. Checked directly against the live Henry Ford SHELL model's raw `Assembly Code` property (not our derived `Turner UF Code` output) via Speckle Admin — confirmed only 4 distinct raw values exist across all 604 elements (`C1010145`×100, `B2010160`×73, `B2020200`×24, `B2010`×2), all ASTM-suffix or bare-section codes, none dot-notation or collapsed-dot form. Also confirmed there's no second/alternate parameter holding a differently-formatted code — `Assembly Code` is the only Assembly/Uniformat-related path present. So "0" is genuinely correct for this dataset; the collapsed-dot normalisation path (`try_normalise_to_level4`) was already wired in correctly and just has nothing to normalise here. While checking, found a real gap worth fixing regardless: `LEVEL4_PATTERN`/`ASTM_CODE_PATTERN` only match an uppercase leading letter, so a code authored lowercase (e.g. a fat-fingered `b2010.10`) would silently fail `is_level4_coded` and get treated as unrecognised legacy needing re-prediction, even though it's already correct. Fixed by uppercasing in `get_assembly_code()` on ingestion — safe and unconditional, Turner/ASTM codes have no legitimate lowercase form. Added `tests/test_get_assembly_code.py` covering lowercase Level 4, lowercase ASTM, lowercase collapsed-form (still normalises correctly), whitespace, and blank-value cases. |
| 2026-08-13 | **Curtain wall category heuristic was overriding a wall's own legacy code section.** Spot-checked one of the 24 Curtain Systems predictions in the viewer: `B2010160` (a currently correct legacy code, matching B2010.40) sat right next to another wall coded `B2020200` that was *also* getting predicted `B2010.40` — but `B2020` is Turner's "Exterior Windows" section, a different section entirely from `B2010` "Exterior Walls." Confirmed via the fixture that Turner does distinguish true curtain wall (B2010.40, described with "curtain wall assemblies," "glazed aluminum curtain walls") from storefront/window-wall systems (B2020.30 "Exterior Window Wall," described with "storefronts, aluminum" and "window wall, aluminum") — but Revit's generic "Curtain Systems"/"Curtain Panels"/"Curtain Wall Mullions" categories can't tell these apart, and the heuristic was blindly forcing B2010.40 regardless of what a wall's own pre-existing legacy code said. Documented the full Level 1–5 hierarchy in `codes.py` while investigating this (confirmed directly against the fixture — Level 3 = bare section like `B2020`, Level 4 = dot sub-code like `B2020.30`, Level 5 = quantity-takeoff line items, not modelled). Fix: `predict._heuristic_signals()` now checks `codes.legacy_code_section()` — if a curtain-category wall's own legacy code section disagrees with B2010, crosswalk to `B2020.30` instead of defaulting to `B2010.40`. Deliberately capped at `CURTAIN_LEGACY_CROSSWALK_CONFIDENCE = 0.65` (Tier 2, "suggest but verify") rather than Tier 1 — B2020.30 is a plausible domain read (Turner's own fixture text supports it), not a confirmed ASTM-to-Turner crosswalk, since no such crosswalk table exists in the source data. New method value `heuristic_category_crosswalk` gets its own viewer annotation bucket ("window wall crosswalk — verify") distinct from the confident `heuristic_category` bucket. Added `tests/test_predict_codes.py::TestCurtainWallLegacyCodeCrosswalk` (disagreeing section crosswalks, agreeing section stays B2010.40, no legacy code stays B2010.40). |
...
| 2026-08-13 | **Raised TIER_2_THRESHOLD from 0.50 to 0.55 — a bare 50% match is a coin toss, not "quick check."** Feedback: Tier 2 should require more than a lone, uncorroborated `heuristic_name` keyword match landing exactly at its base confidence (0.50) — that's a coin toss, and belongs in Tier 3 ("needs a closer look") alongside the `default` no-signal-at-all case, not Tier 2 ("suggest but verify"). Structural reason this specific band was always exactly 0.50, never higher or lower: `heuristic_name` can only ever be the primary/`signals[0]` value in `_heuristic_predict()` when neither category nor Function fired first — meaning whenever it *is* primary, there's nothing left in `others` to corroborate or conflict it away from its raw `METHOD_CONFIDENCE["heuristic_name"]` value. Moved `TIER_2_THRESHOLD` to 0.55, just above that band. Knock-on effect worth flagging: this also demotes the one case `CURTAIN_LEGACY_CROSSWALK_CONFIDENCE` (0.65) can be pushed down to by `CONFLICT_PENALTY` (0.65 − 0.15 = 0.50) — a hardcoded B2020 crosswalk guess that *also* contradicts the wall's own Function parameter now correctly lands Tier 3 instead of Tier 2. This is a live, non-hypothetical case: the 24 real `B2020200` Curtain Systems elements from the 2026-08-13 investigation above will land Tier 3 on the next run if any of them have a Function param that disagrees with the B2020.30 crosswalk (not yet re-verified against the live model — was working from the crosswalk logic, not a fresh SQL check). Updated `TestConfidenceToTier` boundaries (`0.55`→2, `0.54`/`0.50`→3) and added two new behavioural tests: `TestPerObjectConfidenceAdjustment::test_bare_name_only_match_is_a_coin_toss_and_lands_tier_3` (bare `heuristic_name` → Tier 3) and `TestCurtainWallLegacyCodeCrosswalk::test_disagreeing_section_crosswalk_that_also_conflicts_with_function_drops_to_tier_3` (the crosswalk-conflict case, mirroring the real 24-element scenario). All 50 offline tests pass. |
